...
async function callModelGeneric(userPrompt: string, systemPrompt: string): Promise<string> {
  const errors: string[] = [];
  const isDev = process.env.NODE_ENV === 'development';
  ...
  try {
    ...
    if (isDev) console.log('[AI:meeting] gemini-2.0-flash ✓');
    return result.response.text();
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    errors.push(`gemini-2.0-flash: ${msg.slice(0, 120)}`);
    if (isDev) console.warn('[AI:meeting] gemini-2.0-flash ✗', msg);
  }
  ...
  try {
    ...
    if (isDev) console.log('[AI:meeting] gemini-1.5-flash ✓');
    return result.response.text();
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    errors.push(`gemini-1.5-flash: ${msg.slice(0, 120)}`);
    if (isDev) console.warn('[AI:meeting] gemini-1.5-flash ✗', msg);
  }
  ...
  if (process.env.GROQ_API_KEY) {
    try {
      ...
      if (isDev) console.log('[AI:meeting] groq ✓');
      return text;
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
...
export async function summarizeMeeting(rawNotes: string): Promise<string> {
...
export async function extractActionItems(rawNotes: string): Promise<ExtractedActionItem[]> {
  const raw = await callModelGeneric(rawNotes, MEETING_EXTRACT_SYSTEM);
  const cleaned = raw.replace(/```json\s*/gi, '').replace(/```\s*/g, '').trim();
  const parsed = JSON.parse(cleaned);
  if (!Array.isArray(parsed)) throw new AIParseError('Expected JSON array for action items');
  return parsed;
}
